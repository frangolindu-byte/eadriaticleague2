...
def get_today_date():
    """Retorna date de hoje."""
    return datetime.now().date()


def extract_date_from_liga(liga):
    """Extrai data do nome da liga.

    Exemplo: 'FC26 R324(CHAMPIONS LEAGUE)03.04.2026' -> date(2026, 4, 3)
    Retorna None se nao encontrar data valida.
    """
    match = re.search(r'(\d{2})\.(\d{2})\.(\d{4})', liga)
    if match:
        d, m, y = match.groups()
        try:
...
def match_cross_key(m):
    """Gera chave unica para um confronto (independente de quem e mandante)."""
    players = tuple(sorted([m["p1"], m["p2"]]))
    return players + (m.get("liga", ""),)
...
def find_alerts(matches):
    """Encontra melhores jogos FUTUROS do dia com score alto.

    Filtros aplicados:
    1. Status == Agendado
    2. Data da liga >= hoje
    3. Cross-reference: ignora se o mesmo confronto ja tem resultado Finalizado
    4. Deduplicacao: cada confronto e analisado apenas uma vez
    """
    ps = player_stats(matches)
    today = get_today_str()
    today_date = get_today_date()

    # ── Passo 1: Mapear jogos ja finalizados para cross-reference ─────
    finished_keys = set()
    for m in matches:
        if m["status"] == "Finalizado":
            finished_keys.add(match_cross_key(m))
    print(f"  Confrontos finalizados no historico: {len(finished_keys)}")

    # ── Passo 2: Filtrar apenas jogos futuros ainda nao jogados ───────
    all_agendados = [m for m in matches if m["status"] == "Agendado"]
    print(f"  Total de jogos com status Agendado: {len(all_agendados)}")

    upcoming = []
    filtered_date = 0
    filtered_finished = 0
    for m in all_agendados:
        liga = m.get("liga", "")

        # Filtro de data: apenas jogos de hoje ou futuro
        match_date = extract_date_from_liga(liga)
        if match_date and match_date < today_date:
            filtered_date += 1
            continue

        # Cross-reference: ignorar se ja existe resultado finalizado
        ck = match_cross_key(m)
        if ck in finished_keys:
            filtered_finished += 1
            continue

        upcoming.append(m)

    print(f"  Filtrados por data passada: {filtered_date}")
    print(f"  Filtrados por ja finalizado: {filtered_finished}")
    print(f"  Jogos futuros restantes: {len(upcoming)}")

    # ── Passo 3: Deduplicar confrontos (P1vsP2 = P2vsP1) ─────────────
    seen_confrontos = set()
    unique_upcoming = []
    for m in upcoming:
        ck = match_cross_key(m)
        if ck not in seen_confrontos:
            seen_confrontos.add(ck)
            unique_upcoming.append(m)

    dedup_count = len(upcoming) - len(unique_upcoming)
    if dedup_count > 0:
        print(f"  Confrontos duplicados removidos: {dedup_count}")
    print(f"  Confrontos unicos para analise: {len(unique_upcoming)}")

    # ── Passo 4: Pontuar e gerar alertas ──────────────────────────────
    alerts = []
...
def send_alerts():
    ...
    for a in alerts:
        ...
        if k not in sent_keys:
            a["key"] = k
            new.append(a)
            sent_keys.add(k)  # Evita duplicatas dentro da mesma execucao
        else:
